...
img_height=50
total_Test_case=12
model_name="svm" #svm ,tree,knn
MODEL_DIR="./models/"+model_name
config="config_"+model_name
min_samples_split=3
num_neighb=20
...
# load dataset
def CALLFunc(train_v,test_v, mask,train_per=80, n_mins=5,svm_degree=3):
    ...
    global  config
    if model_name=="svm":
        clf = SVR(kernel="linear") #" degree=svm_degree) poly sigmoid linear rbf
        config= config+"_"+"linear"
    elif model_name=="tree":
        clf=tree.DecisionTreeRegressor(min_samples_split=min_samples_split)
        config= config+"_"+str(min_samples_split)
    elif model_name=="knn":
        clf = KNeighborsRegressor(n_neighbors=num_neighb)
        config= config+"_"+str(num_neighb)
    ...
    for i in range(total_Test_case):
        ...
        # pyplot.savefig(MODEL_DIR + "/Logs/" + str(i) + ".png")
